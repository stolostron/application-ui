...
describe("computeNodeStatus ", () => {
  it("return computeNodeStatus generic node green - volume claim bound", () => {
    expect(computeNodeStatus(persVolumePendingStateGreen)).toEqual("green");
  });
...
describe("setResourceDeployStatus 2 ", () => {
  ...
  it("setResourceDeployStatus deployed as green", () => {
    expect(
      setResourceDeployStatus(persVolumePendingStateGreen, [], {})
    ).toEqual(persVolumePendingStateGreenRes);
  });

  it("return persistent volume node yellow - volume claim pending", () => {
    expect(
      setResourceDeployStatus(persVolumePendingStateYellow, [], {})
    ).toEqual(persVolumePendingStatePendingRes);
  });
});
