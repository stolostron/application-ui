...
class StatusCellMemory extends React.Component {
  ...
  updateChartData(props) {
    ...
    const values = clusters.reduce((acc, {capacity, usage}) => {
      data['available'] += inflateKubeValue(capacity.memory)
      data['used'] += inflateKubeValue(usage.memory)
      return acc
    }, data)
  ...
  render() {
    ...
    return (
      <div>
        <StatusChartCell
          header={msgs.get('overview.status.memory', locale)}
          tagValue={tagValue}
          chartData={chartData}
...
export default StatusCellMemory
