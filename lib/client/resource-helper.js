...
export const getNumDeployments = data => {
  if (
    data &&
    data.custom &&
    data.custom.deployments &&
    data.custom.deployments instanceof Array
  ) {
...
export const getNumPendingDeployments = data => {
  if (
    ...
    data.related &&
    data.related.length > 0 &&
    data.related[0].items
  ) {
    return data.related[0].items.filter(obj => obj.status === 'Pending').length
  } else {
    return '-'
  }
}
...
export const getNumInProgressDeployments = data => {
  if (
    ...
    return data.related[0].items.filter(obj => obj.status === 'In Progress').length
  } else {
    return '-'
  }
}
...
export const getNumFailedDeployments = data => {
  if (
    ...
    return data.related[0].items.filter(obj => obj.status === 'Failed').length
  } else {
    return '-'
  }
}
