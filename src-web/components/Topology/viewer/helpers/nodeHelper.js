...
export default class NodeHelper {
  ...
  updateNodeIcons = () => {
    ...
    const svgIcons = nodes
      ...
      .data(({layout: {nodeIcons}}) => {
        return nodeIcons ? Object.values(nodeIcons).filter(({icon})=>{
          return !!icon
        }) : []
      })
    svgIcons.enter()
    ...
    // png icons
    nodes
      .selectAll('image.nodeIcon')
      .remove()
    const pngIcons = nodes
      .selectAll('image.nodeIcon')
      .data(({layout: {nodeIcons}}) => {
        return nodeIcons ? Object.values(nodeIcons).filter(({href})=>{
          return !!href
        }) : []
      })
    pngIcons.enter()
    ...
    nodes.selectAll('use.shape')
      .classed('disabled', ({layout: {isDisabled}})=>{
        return isDisabled
      })
  ...
  moveNodes = (transition, currentZoom, searchChanged) => {
    ...
    nodeLayer
      ...
      .attrs(({dx, dy, width, height}, i, ns) => {
        const {layout: {x=0, y=0, scale=1}} = d3.select(ns[i].parentNode).datum()
        return {
  ...
  dragNode = (d, i, ns) => {
    ...
    if (Math.hypot(layout.x - layout.undragged.x, layout.y - layout.undragged.y) > 5) {
      ...
      // drag status message
      node.selectAll('text.nodeStatus')
        .attrs(({textBBox:{dy}}, i, ns) => {
          const {layout: {x, y}} = d3.select(ns[i].parentNode).datum()
          return {
            'x': x,
            'y': y + dy
  ...
  svg.select('g.nodes').selectAll('g.node')
    .classed('selected', (node)=>{
      ...
      layout.selected = selected &&
        _.get(selected, 'id') === _.get(node, 'id')
...
export const moveLabels = (svg) => {
  ...
  nodeLayer
    .selectAll('text.nodeStatus')
    .attrs((l, i, ns) => {
      const {layout: {x, y, textBBox, scale=1}} = d3.select(ns[i].parentNode).datum()
