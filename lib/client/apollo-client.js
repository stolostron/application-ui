...
class apolloClient {
  ...
  remove(input) {
    if (input.kind && input.kind === 'release') {
      return client.mutate({ mutation: Query.deleteHelm, variables: { name: input.name, namespace: input.namespace, cluster: input.cluster } })
    }
    if (input.selfLink) {
      return client.mutate({ mutation: Query.deleteResource, variables: { selfLink: input.selfLink, childResources: input.childResources } })
    } else {
