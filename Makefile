...
run:
	...
	--network mcm-network \
	-d -p $(HOST):$(APP_PORT):$(CONTAINER_PORT) $(DOCKER_IMAGE):$(DOCKER_BUILD_TAG)
...
ifeq ($(OSARCH), linux-amd64)
	...
	docker pull $(IMAGE_REPO)/search-api-amd64
	docker run \
	-e NODE_ENV=test \
	-e MOCK=true \
	--name search-api \
	--network mcm-network \
	--ip 10.10.0.6 \
	-d -p 127.0.0.1:4010:4010 $(IMAGE_REPO)/search-api-amd64
