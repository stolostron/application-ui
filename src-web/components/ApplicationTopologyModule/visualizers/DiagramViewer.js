...
class DiagramViewer extends React.Component {
    ...
    this.setState((prevState, props) => {
      ...
      const previousLinks = prevState.links.filter(link => {
        ...
        if (!nodeMap[source] || !nodeMap[target]) {
          ...
          if (
            props.reloading ||
            (!nodeMap[source].isDesign && !nodeMap[source].isDesign)
          ) {
  ...
  setDiagramRefreshContainerRef = ref => {
    this.diagramRefreshContainerRef = ref
  };
  setDiagramRefreshTimeRef = ref => {
    this.diagramRefreshTimeRef = ref
  };
  getZoomHelper = () => {
  ...
  generateDiagram() {
    ...
    this.layoutHelper.layout(
      ...
      layoutResults => {
        ...
        if (
          this.diagramOptions.showSectionTitles !== false &&
          (titles.length ||
            searchChanged ||
