...
const typeDefs = gql`
  ...
  type Mutation {
    ...
    updateSingleQueryTab(openedTabName: String, description: String, searchText: String, updateUnsavedOrExisting: Boolean): JSON
    ...
    Mutation: {
      ...
      updateSingleQueryTab: (_, variables, { cache }) => {
        ...
        if (variables.updateUnsavedOrExisting) {
          const index = tabs.findIndex(tab => tab.searchText === variables.searchText)
          if (index !== -1) {
      ...
      removeSingleQueryTab: (_, variables, { cache }) => {
        ...
        if (index !== -1 && tabs.length > 1) {
          ...
          } else if (data.searchQueryTabs.openedTabName === variables.queryName) {
