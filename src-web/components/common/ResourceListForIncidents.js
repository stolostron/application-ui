...
import { changeTablePage, searchTable, sortTable, fetchResourcesForIncidents, updateSecondaryHeader } from '../../actions/common'
...
class ResourceListForIncidents extends React.Component {
  ...
  componentWillMount() {
    ...
    const { fetchResourcesForIncidents, selectedFilters=[]} = this.props
    fetchResourcesForIncidents(selectedFilters,this.props.params.name || 'gbapp', this.props.params.namespace)
  }

  componentWillReceiveProps(nextProps) {
    if (nextProps.selectedFilters !== this.props.selectedFilters) {
      this.setState({ xhrPoll: false })
      this.props.fetchResourcesForIncidents(nextProps.selectedFilters)
    }
  }
  ...
  reload() {
    if (this.props.status === REQUEST_STATUS.DONE) {
      ...
      const { fetchResourcesForIncidents, selectedFilters=[], name, namespace } = this.props
      fetchResourcesForIncidents(selectedFilters, this.props.params.name || 'gbapp', this.props.params.namespace)
    }
  }
...
const mapDispatchToProps = (dispatch, ownProps) => {
  ...
  return {
    fetchResourcesForIncidents: (selectedFilters, params) => {
      //TODO: searchTable if customized tags
      dispatch(fetchResourcesForIncidents(resourceType, combineFilters(selectedFilters), params.name || 'gbapp', params.namespace))
    },
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(ResourceListForIncidents))