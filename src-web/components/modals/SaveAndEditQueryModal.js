...
class SaveAndEditQueryModal extends React.PureComponent {
  ...
  makeApolloQuery = (name, description, searchText, client) => {
    apolloClient
      .saveUserQueries({ name, description, searchText })
      .then(result => {
        if (
          result.data.saveQuery.errors &&
          result.data.saveQuery.errors.length > 0
        ) {
          this.setState({
            errorMessage: result.data.saveQuery.errors[0],
            loading: false
          })
        } else {
          this.setState({
    ...
    client.mutate({
      mutation: UPDATE_SINGLE_QUERY_TAB,
      variables: { ...newData }
    })
  };

  isSubmitDisabled = () =>
    _.isEmpty(this.state.nameInput) ||
    (_.isEmpty(this.state.searchText) && !this.state.editExisting);
          ...
          return (
            <div className={'save-and-edit-query-modal'}>
              ...
              >
                <div>
                  {this.state.loading && (
                    <Loading className="content-spinner" />
                  )}
                  <p className={'save-modal-description'}>
                    {msgs.get(
                      'modal.query.add.description',
                      this.context.locale
                    )}
                  </p>
                  ...
                  <TextInput
                    id="add-query-name"
                    labelText={msgs.get(
                      'modal.query.add.name.label',
                      this.context.locale
                    )}
                    placeholder={msgs.get(
                      'modal.query.add.name',
                      this.context.locale
                    )}
                  ...
                  <TextArea
                    id="add-query-desc"
                    labelText={msgs.get(
                      'modal.query.add.desc.label',
                      this.context.locale
                    )}
                    placeholder={msgs.get(
                      'modal.query.add.desc',
                      this.context.locale
                    )}
