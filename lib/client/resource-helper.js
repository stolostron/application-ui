...
export const getSearchLink = (params = {}) => {
  const { properties, showRelated, clusterNames } = params
  let textsearch = ''
  let isArgoApp = false
  if (properties && properties.apigroup) {
    isArgoApp = properties.apigroup.indexOf('argo') !== -1
  }
  if (isArgoApp && clusterNames && clusterNames.length > 0) {
    textsearch = `kind:cluster name:${clusterNames.join()}`
  } else {
