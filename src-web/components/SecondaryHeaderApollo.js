...
import {
  UPDATE_QUERY_TABS,
  REMOVE_SINGLE_QUERY_TAB
} from '../apollo-client/queries/StateQueries'
...
export class SecondaryHeaderApollo extends React.Component {
  ...
  handleMouseHover = () => {
    this.setState(preState => {
      return { isHovering: !preState.isHovering }
    })
  };
            ...
            >
              <div className="secondary-header apollo">
                {tabs && tabs.length > 0 ? (
                  ...
                  >
                    <Tabs
                      className="secondary-header-tabs"
                      selected={
                        this.getSelectedTab(tabs, openedTabId, client) || 0
                      }
  ...
  renderAddNewTab = (client, unsavedCount, tabs, locale) => {
    if (tabs.length < 10) {
      return (
        <Tab
          label={msgs.get('tabs.add.new', locale)}
          className={'header-tab-add-new'}
          id={'add-new-query'}
          onClick={this.handleClickNewTab(client, unsavedCount, tabs, locale)}
          subcomponent={
            <Icon
      ...
      return (
        <div className="header-tab-add-new inactive">
          {msgs.get('tabs.add.new', locale)}
        </div>
      )
    }
  };
  ...
  renderTabs(client, tabs, unsavedCount) {
    ...
    return result.map(tab => {
      if (!tab) {
        return <div className="header-tab-divider" />
      }
      return (
        <Tab
          label={`${tab.queryName} ${tab.updated ? '*' : ''}`}
          key={tab.id}
          id={tab.id}
          className={
            tabs.length === 1
              ? 'header-tab-container single-tab'
          ...
          subcomponent={
            tabs.length > 1 ? (
              <Icon
                ...
                onClick={this.handleRemoveClick(client, tabs, tab)}
              />
            ) : null
          }
        />
  ...
  handleRemoveClick = (client, tabs, tab) => async evt => {
    if (evt.which === undefined || evt.which === 13 || evt.which === 32) {
      ...
      await client.mutate({
        mutation: REMOVE_SINGLE_QUERY_TAB,
        variables: { ...newData }
      })
  ...
  getSelectedTab(tabs, openedTabId, client) {
    const currentTab = tabs.filter(
      tab =>
        tab.id === openedTabId &&
        !tab.queryName.toLowerCase().includes('unsaved')
    )
    const result = tabs.reduce((r, a) => r.concat(a, 0), [])
    const index = result.findIndex(tab => tab.id === openedTabId)
