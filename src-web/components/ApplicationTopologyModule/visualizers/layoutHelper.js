...
export default class LayoutHelper {
  ...
  layout = (nodes, links, hiddenLinks, options, cb) => {
    ...
    const groups = this.getNodeGroups
      ...
      : this.getNodeGroupsDefault(nodes)
          ...
          connected => {
            ...
            if (!isMultiCluster && canBeConsolidated(nodeMap)) {
              if (!consolidatedGroup) {
                ...
                consolidatedGroup.nodeMap = Object.assign(
                  consolidatedGroup.nodeMap,
                  nodeMap
                )
                consolidatedGroup.details.edges = consolidatedGroup.details.edges.concat(
                  edges
                )
  ...
  cloneNodes = (groups, nodes) => {
    ...
    if (Object.keys(this.nodesToBeCloned).length) {
      ...
      this.shapeTypeOrder.forEach(type => {
        if (nodeGroups[type] && nodeGroups[type].connected) {
          nodeGroups[type].connected.forEach(
            ({ nodeMap, details: { edges } }) => {
      ...
      if (nodeGroups[type]) {
        ...
        for (var clusterName in detailMap) {
          ...
          if (nodes.length > 48) {
            nodes.sort(
              (
                { layout: { label: a = '', uid: au, newComer: an } },
                { layout: { label: b = '', uid: bu, newComer: bn } }
              ) => {
                if (!an && bn) {
                  return -1
                } else if (an && !bn) {
  ...
  filterByType = (nodes, links) => {
    return this.filterHelper.filterByType(
      nodes,
      links,
  ...
  filterByName = (cy, collections) => {
    const { searchNames, directedPath } = this.filterHelper.filterByName(
      cy,
      collections,
      this.searchName,
      {
  ...
  layoutCollections = (collections, hiddenLinks) => {
    ...
    const clayouts = []
    collections.forEach(
      ({ elements, details, hashCode, type, options: { name } }) => {
    ...
    if (
      ...
      hashCodeToPositionMap = this.sortCollection(
        clayouts,
        this.rowPositionCache
      )
    }
