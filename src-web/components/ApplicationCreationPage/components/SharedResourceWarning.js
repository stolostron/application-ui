...
const getQuery = (resourceType, name, namespace) => {
  ...
  return {
    ...
    relatedKinds:
      kind === 'subscription'
        ? CHILD_RESOURCE_TYPES.map(t => t.toLowerCase())
        : ['application']
  }
}
...
const SharedResourceWarning = ({ resourceType, control, locale }) => {
  ...
  const [prNamespace, prName] = getResourceNameAndNamespace(
    ...
    () => {
      if (control.editMode && resourceName && resourceNamespace) {
        ...
        apolloClient
          ...
          .then(response => {
            const hostingSubscription = _.get(
              response,
              'data.searchResult[0].items[0]._hostingSubscription'
            )
            ...
            <p
              ...
              ${msgs.get(
                  ...
                  [
                    ...
                    getCodeSpan(deployingSubscription)
                  ],
                  locale
                )}
              `
