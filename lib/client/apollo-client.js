...
class apolloClient {
  ...
  getResourceFromSearch(resourceType, variables = {}) {
    return searchClient.query({ query: lodash.get(Query, resourceType.name), variables })
  }
  ...
  remove(input) {
    ...
    if (input.selfLink) {
      return client.mutate({
        ...
        //   console.log('delete resource is ' + JSON.stringify(deleteResource))
        ...
        //   const { searchResult } = cache.readQuery({ query: SEARCH_QUERY, variables: { input: [convertStringToQuery(`kind: application`)]}})
        //   cache.writeQuery({
        //     query: SEARCH_QUERY,
        //     variables: { input: [convertStringToQuery(`kind: application`)] },
