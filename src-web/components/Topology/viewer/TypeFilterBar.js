...
import '../scss/diagram-filter-bar.scss'
import msgs from '../../../../nls/platform.properties'
import _ from 'lodash'

export default class TypeFilterBar extends React.Component {
  ...
  handleClick = (type) => {
    this.setState((prevState) => {
      ...
      saveActiveTypeFilters(this.props.typeFilterCookie,
        activeFilters, this.props.availableFilters)
      ...
      <div className='topology-type-filter-bar' role='region' aria-label={typeFilterTitle} id={typeFilterTitle}>
        <div className='filter-bar'>
          {availableFilters.map(type => {
            let displayName = msgs.get(`filterbar.type.${type}`, locale)
    ...
    return (
      <div className='filter-bar-button-container' key={label} title={tooltip||''}>
        <div className='filter-bar-button'
          ...
          aria-label={msgs.get(selected ? 'select' : 'unselect', locale)}
