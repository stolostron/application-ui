...
class ControlPanelCards extends React.Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div className='creation-view-controls-card-container'
...
const ControlPanelCard = ({choice, handleOnClick, type, selected, locale}) => {
  ...
  return <div className={wrapperClasses} id={id} role='button' onClick={handleClick}
    disabled={true} tabIndex="0" aria-label={title} onKeyDown={handleClick}>
    <div className={'mcx--provider-card-container'}>
      <div className={cardClasses}>
        <img src={`${config.contextPath}/graphics/${logo}`}  alt={title} />
        <p className={'mcx--create-cluster-page__provider-card-title'}>{title1}<span>{title2}</span></p>
      </div>
      {selected && <div className='card-checkmark'>
