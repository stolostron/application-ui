...
const getCIDRContextTexter = (cidrFieldKey, sourcePath) => {
  ...
  return (value, templateObjectMap, locale) => {
    ...
    if (cidr.isValid() && !cidr.contains(value)) {
      ...
      return msgs.get('creation.ocp.cluster.valid.cidr.membership', [cidrFieldName, cidrString], locale)
    }
    return null
  }
}
...
export const VALIDATE_USER_AND_IP = {
  tester: new RegExp('^[-.0-9a-z]+@(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]).){3,4}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(:[0-9]+)*$'),
