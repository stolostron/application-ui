...
class AddRepoActionAndModal extends React.PureComponent {
  ...
  handleModalOpen = () => {
    this.setState({
      ...
      nameInput: '',
      urlInput: '',
      urlInputValidationMsg: undefined
    })
  };
  handleModalCancel = () => {
  ...
  handleModalSubmit = () => {
    if (this.validateUrlInput()) {
      this.props
        .onAddRepo(this.state.nameInput, this.state.urlInput)
        .then(() => {
          this.setState({
            modalOpen: false,
            nameInput: '',
  ...
  isSubmitDisabled = () =>
    this.state.nameInput === '' || this.state.urlInput === '';
  ...
  validateUrlInput = () => {
    ...
    if (this.state.urlInput && this.state.urlInput.match(urlPattern) == null) {
      this.setState({
        urlInputValidationMsg: `${msgs.get('helmRepo.urlformat', this.context)}`
      })
      return false
    }
...
const mapDispatchToProps = dispatch => {
  return {
    onAddRepo: (Name, URL) =>
      dispatch(createResource(RESOURCE_TYPES.HCM_REPOSITORIES, { Name, URL }))
  }
}
