...
const positionApplicationRows = (row, typeToShapeMap) => {
  ...
  positionRowsDown(0, 0, row, placedSet, positionMap, deployableList, typeToShapeMap, placeLast)
...
const positionRowsDown = (idx, y, row, placedSet, positionMap, deployableList, typeToShapeMap, placeLast, offsetRow=0) => {
  if (row.length) {
    ...
    // place next row down
    y += NODE_SIZE * 2.4
    positionRowsDown(idx + 1, y, nextRow, placedSet, positionMap, deployableList, typeToShapeMap, placeLast, hybridRow?width/2:0)
    if (hybridRow) {
      y += NODE_SIZE * 2.4
      positionRowsDown(idx + 1, y, clusterList, placedSet, positionMap, deployableList, typeToShapeMap, placeLast)
    }
  }
}
