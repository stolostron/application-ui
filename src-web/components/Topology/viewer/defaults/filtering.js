...
const TypeFilters = {
  cluster: {
    filterTypes: {clusterStatuses: 'clusterStatuses', providers: 'providers' , purpose: 'purpose', region: 'region' , k8type: 'k8type' },
    searchTypes: new Set(),
  },
  weave: {
    filterTypes: {podStatuses: 'podStatuses', hostIPs: 'hostIPs',  namespaces: 'namespaces',  labels: 'labels' },
  ...
  policy: {
    filterTypes: {providers: 'providers' , purpose: 'purpose', region: 'region' , k8type: 'k8type' },
    searchTypes: new Set(),
  },
}
...
export const getAllFilters = (mode, typeToShapeMap, isLoaded, nodes, options={}, activeFilters, knownTypes, userIsFiltering, locale ) => {
  ...
  if (availableTypes) {
    ...
    otherTypeFilters = []
    availableTypes = sorted.filter(a=>{   // anything w/o a shape is other automaically
      if (!typeToShapeMap[a]) {
      ...
      if (idx>7) { // then if the toolbar will be too full, put the rest in other
        otherTypeFilters.push(a)
        return false
      }
      return true
...
const addAssortedAvailableFilters = (mode, availableFilters, activeFilters, nodes=[], locale) => {
  if (nodes.length>0) {
    ...
    case 'weave':
      return addAvailableRelationshipFilters(mode, availableFilters, activeFilters, nodes, locale)

    case 'application':
      return addAvailableRelationshipFilters(mode, availableFilters, activeFilters, nodes, locale)

    case 'policy':
      return addAvailablePolicyFilters(availableFilters, activeFilters, nodes, locale)
    }
  }
}
  ...
  Object.keys(filterTypes).forEach(type=>{
    ...
    case 'clusterStatuses':
      ...
      availableSet=new Map([
        ['recent', msgs.get('topology.filter.category.status.recent', locale)],
        ['offline', msgs.get('topology.filter.category.status.offline', locale)],
        ['violations', msgs.get('topology.filter.category.status.violations', locale)],
      ])
      break
    case 'providers':
...
const addAvailableRelationshipFilters = (mode, availableFilters, activeFilters, nodes, locale) => {
  ...
  Object.keys(filterTypes).forEach(type=>{
    ...
    switch (type) {
    case 'podStatuses':
      name = showPods && msgs.get('topology.filter.category.podStatuses', locale)
      availableSet=new Map([
        ['recent', msgs.get('topology.filter.category.status.recent', locale)],
        ['restarts', msgs.get('topology.filter.category.status.restarts', locale)],
        ['pending', msgs.get('topology.filter.category.status.pending', locale)],
  ...
  let hasPods = false
  const {podStatuses=new Set(), hostIPs=new Set(), namespaces=new Set()} = activeFilters
  nodes.forEach(node=>{
      ...
      if (activeTypes.has(type) || activeTypes.has('other')) {
        ...
        Object.keys(filterTypes).forEach(filterType=>{
          ...
          if (filter) {
            switch (filterType) {
            case 'hostIPs':
              if (podStatus && podStatus.hostIPs.size>0 && hasPodStatus(filterType, podStatus, podStatuses)) {
            ...
            case 'labels':
              if (labels && hasPodStatus(filterType, podStatus, podStatuses, hostIPs) &&
...
const filterClusterNodes = (nodes, activeFilters) => {
  const { clusterStatuses, type, purpose=new Set(), providers=new Set(), region=new Set(), k8type=new Set() } = activeFilters
  ...
  return nodes.filter(node=>{
    ...
    if (hasType && type==='cluster') {
      ...
      if (clusterStatuses && clusterStatuses.size>0) {
        ...
        if (!((clusterStatuses.has('offline') && isOffline) ||
...
const filterRelationshipNodes = (nodes, activeFilters, availableFilters, mode) => {
  const { type, podStatuses=new Set(), hostIPs=new Set(), namespaces=new Set(), labels=new Set() } = activeFilters
  ...
  return nodes.filter(node=>{
    ...
    if (!hasType && includeOther && !ignoreNodeTypes.has(type) && !availableTypeSet.has(type)) {
      hasType = true
    }
    ...
    if (podStatuses.size !== 0) {
      ...
      if (podStatus) {
        const {hasPending, hasFailure, hasRestarts, isRecent} = podStatus
        hasPodStatus = ((podStatuses.has('failed') && hasFailure) ||
            (podStatuses.has('pending') && hasPending) ||
            (podStatuses.has('restarts') && hasRestarts) ||
            (podStatuses.has('recent') && isRecent))
      }
    ...
    // filter namespaces
    const hasNamespace = namespaces.size === 0 || namespaces.has(namespace||'<none>')
