...
export function getNodeGroups(nodes) {
  ...
  nodes.forEach(node => {
    ...
    node.layout = Object.assign(node.layout || {}, {
      ...
      compactLabel: getWrappedNodeLabel(
        label.replace(/[0-9a-fA-F]{8,10}-[0-9a-zA-Z]{4,5}$/, '..'),
        12,
        2
      )
    })
    ...
    default:
      if (hasPods) {
        description = msgs.get(
          'topology.controller.pods',
          [node.type, layout.pods.length],
          locale
        )
      } else if (hasService) {
        description = msgs.get(
          'topology.service.controller',
          [node.type],
          locale
        )
      }
...
function getNodeTooltips(node, locale) {
  ...
  if (type === 'pod') {
    ...
    const href = kind
      ? `${contextPath}/search?filters={"textsearch":"kind:${kind} name:${name}"}`
      : undefined
...
function addNodeDetails(node, details, podOnly) {
  ...
  if (labels && labels.length) {
    ...
    labels.forEach(({ name: lname, value: lvalue }) => {
      const labelDetails = [{ value: `${lname} = ${lvalue}` }]
  ...
  if (podModel) {
    ...
    details.push({
      type: 'logs',
      value: {
        resourceType: RESOURCE_TYPES.HCM_PODS,
...
export function getConnectedLayoutOptions(
  { elements, details: { isConsolidation } },
  options
) {
  const {
    numOfSections,
  ...
  const useDAG =
    (directedSearchPath && numNodes < 8) || // using search with >
