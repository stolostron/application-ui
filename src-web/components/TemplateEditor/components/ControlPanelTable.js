...
import {ControlMode} from '../../TemplateEditor/utils/initialize-form'
...
class ControlPanelTable extends React.Component {
  ...
  getRows(){
    ...
    if (selectedKey) {
      items = sortTable? sortTable(items, selectedKey, sortDirection, active ) :
    ...
    return items.map((item, inx) => {
      ...
      if (deletePrompt) {
        row.action = (
          <div className='creation-view-controls-table-delete-button' tabIndex='0' role={'button'}
            title={text} aria-label={text}
            onClick={handleDeleteRow} onKeyPress={handleDeleteRowKey}>
            <svg className='icon'>
    ...
    return (
      <div className='creation-view-controls-table-container'
        ...
        <PaginationV2
          ...
          pageRangeText={(current, total) => msgs.get('pagination.pageRange', [current, total], locale)}
          itemRangeText={(min, max, total) => `${msgs.get('pagination.itemRange', [min, max], locale)} ${msgs.get('pagination.itemRangeDescription', [total], locale)}`}
  ...
  renderTree(rows) {
    ...
    if (isFailed) {
      ...
      return <DataTableSkeleton columnCount={headers.length-1} compact={false} rowCount={3}
        showheader={'true'} showtoolbar={'true'} zebra={false} />
    } else {
      const {id, exceptions=[]} = control
      const { sortDirection, selectedKey, searchValue, originalSet } = this.state
      const sortColumn = selectedKey
      let {actions} = prompts
      actions = React.Children.map(actions, action => {
        return React.cloneElement(action, {appendTable: this.handleTableAction.bind(this, add), locale})
      })
      ...
      return (
        <DataTable
          ...
          render={({ rows:_rows, headers:_headers}) => {
            return (
              <TableContainer>
                <TableToolbar aria-label={msgs.get('table.toolbar.description', locale)}>
                  <TableToolbarSearch onChange={({target})=>this.setState({searchValue: target.value||'', page:1})}
                ...
                <Table zebra={false}>
                  <TableHead>
                    <TableRow>
                      <TableSelectAll id={'selectAll'}  ariaLabel={'tableSelectAllRow'} name={'tableSelectAllRow'}
                        indeterminate={active.length>0 && active.length<available.length} disabled={false}
                        checked={active.length>0 && active.length===available.length}
                          ...
                          {header.key !== 'action'
                            ? <button
                              ...
                              <Icon
                                ...
                                name='caret--down'
                                description={msgs.get(`svg.description.${!sortColumn || sortDirection === 'desc' ? 'asc' : 'desc'}`, locale)} />
                            </button>
                            : null
                          }
                        </th>
                  ...
                  <TableBody>
                    {_rows.map((row) => (
                      <TableRow key={row.id} className={!originalSet.has(row.id) && 'new-row'}>
                        <TableSelectRow id={row.id} ariaLabel={'tableSelectRow'}  name={'tableSelectRow'}
                          checked={activeSet.has(row.id)} onSelect={this.handleSelect.bind(this, row.id)} />
                        {row.cells.map((cell) => {
                          const key = _.get(cell, 'info.header')
                          const inx = exceptions.findIndex(({cells})=>(cells.indexOf(`${key}-${row.id}`)!==-1))
                          const hasException = inx!==-1
                          const {id:rid} = row
                          return (
                            <TableCell key={cell.id}
                              style={hasException ? {color:'red', cursor:'pointer'} : {}}
                              title={hasException ? exceptions[inx].exception : ''} >
          ...
          <React.Fragment>
            <div className='creation-view-controls-table-singleselect'>
              <DropdownV2
                label={value}
                items={available}
                onChange={this.handleCellEdit.bind(this, rinx, header, 'singleselect')} />
            </div>
          </React.Fragment>
        )
      case 'toggle':
        return (
          <React.Fragment>
            <div className='creation-view-controls-table-toggle'  key={cell.id}>
              {cactive===undefined ? '-'
                : <React.Fragment>
                  <ToggleSmall
                    id={`id${rid}`}
                    ...
                    onToggle={this.handleCellEdit.bind(this, rinx, header, 'toggle')}  />
