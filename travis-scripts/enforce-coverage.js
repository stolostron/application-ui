...
if (
  ...
  console.error(
    'Skipped code coverage enforcement. Unable to find the previous coverage report.'
  )
...
try {
  ...
  console.error(
    'Skipped code coverage enforcement. Unable to parse the previous coverage report.'
  )
  console.error(error)
  process.exit(0) // Ignoring for now, once we get the right process in place we should fail the build.
}
...
if (currReport.total.lines.pct < prevReport.total.lines.pct) {
  result +=
    '\n  - Lines covered declined by: ' +
    (prevReport.total.lines.pct - currReport.total.lines.pct) +
    '%'
}
if (currReport.total.statements.pct < prevReport.total.statements.pct) {
  result +=
    '\n  - Statements covered declined by: ' +
    (prevReport.total.statements.pct - currReport.total.statements.pct) +
    '%'
}
if (currReport.total.functions.pct < prevReport.total.functions.pct) {
  result +=
    '\n  - Functions covered declined by: ' +
    (prevReport.total.functions.pct - currReport.total.functions.pct) +
    '%'
}
if (currReport.total.branches.pct < prevReport.total.branches.pct) {
  result +=
    '\n  - Branches covered declined by: ' +
    (prevReport.total.branches.pct - currReport.total.branches.pct) +
    '%'
}
