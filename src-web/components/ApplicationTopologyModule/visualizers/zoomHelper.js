        ...
        if (this.viewer.viewerContainerContainerRef) {
          ...
          let scale = Math.min(
            ...
            0.95 /
              Math.max(
                width / availableWidth,
                height / (availableHeight - (this.collapsedView ? 50 : 0))
              )
          )
          ...
          if (scale < c.MINIMUM_ZOOM_FIT) {
            ...
            this.viewer.viewerContainerRef.setAttribute(
              'style',
              `height: ${height * scale + c.TOPOLOGY_PADDING}px;`
            )
          } else {
          ...
          const viewerHeight = this.viewer.viewerContainerRef.getBoundingClientRect()
            .height
