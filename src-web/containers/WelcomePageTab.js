...
import {
  ...
  Compliance,
  Users
} from '../components/WelcomeSummaryItems'
...
class Welcome extends React.PureComponent {
  ...
  render() {
    ...
    return (
      <div
        className={`welcome${
          this.hasDynamicSummaryItems() ? ' welcome--condensed' : ''
        }`}
      >
        <div
          className="welcome--introduction"
          role="region"
          aria-label={msgs.get('welcome.label', locale)}
        >
          <div className="welcome--introduction__description">
            <div className="welcome--text">
              <h1>
                {msgs.get('welcome.heading', locale)}{' '}
                <span>{msgs.get('welcome.mcm', locale)}</span>
              </h1>
              <p>
                <span className="welcome--text-secondary">
                  {msgs.get('welcome.paragraph.mcm.title', locale)}
                </span>{' '}
                {msgs.get('welcome.paragraph.mcm.description', locale)}
              </p>
              <p>
                <span className="welcome--text-secondary">
                  {msgs.get('welcome.paragraph.example.title', locale)}
                </span>{' '}
                {msgs.get('welcome.paragraph.example.description', locale)}
              </p>
            </div>
            <div className="welcome--image">
              <img
                src={`${
                  config.contextPath
                }/graphics/MCM-GettingStarted-Header.png`}
                alt={msgs.get('welcome.svg.description.welcome', locale)}
              />
            </div>
          </div>
        </div>
        ...
        <div
          ...
          role="region"
          aria-label={msgs.get('welcome.connect.label', locale)}
        >
          ...
          <div className="welcome--connect__details">
            <div className="welcome--connect__section">
              <img
                className="welcome--img__blog"
                src={`${
                  config.contextPath
                }/graphics/ICP-Connect_Icons-VideoBlog.svg`}
              ...
              <a
                ...
                href="https://www.ibm.com/developerworks/community/wikis/home?lang=en#!/wiki/W1559b1be149d_43b0_881e_9783f38faaff"
                className="bx--link"
                aria-describedby="launchWindow"
              >
                {msgs.get('welcome.connect.box.one.link', locale)}
              </a>
            </div>
            <div className="welcome--connect__section has-divider">
              <img
                className="welcome--img__slack"
                src={`${
                  config.contextPath
                }/graphics/ICP-Connect_Icons-Slack.svg`}
              ...
              <a
                ...
                aria-describedby="launchWindow"
              >
                {msgs.get('welcome.connect.box.two.link', locale)}
              </a>
            </div>
            <div className="welcome--connect__section">
              <img
                className="welcome--img__support"
                src={`${
                  config.contextPath
                }/graphics/ICP-Connect_Icons-Support.svg`}
              ...
              <a
                target="support"
                href="https://ibm.biz/icpsupport"
                className="bx--link"
                aria-describedby="launchWindow"
              >
                {msgs.get('welcome.connect.box.three.link', locale)}
              </a>
            </div>
          </div>
        </div>
  ...
  renderWelcomeSummary() {
    ...
    if (this.hasDynamicSummaryItems()) {
      return (
        <div
          className="welcome--summary--condensed"
          role="region"
          aria-label={msgs.get('welcome.box.label', locale)}
        >
