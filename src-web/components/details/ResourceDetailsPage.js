...
const withResourceDetails = Component => {
  const mapDispatchToProps = dispatch => {
    return {
      updateSecondaryHeader: (title, tabs, breadcrumbItems) =>
        dispatch(updateSecondaryHeader(title, tabs, breadcrumbItems))
    }
  }
    ...
    connect(null, mapDispatchToProps)(
      class extends React.Component {
        ...
        handleSecondaryHeaderUpdate() {
          ...
          const title = this.tabs.includes(segments[segments.length - 1])
          ...
          const tabs = getTabs(
            kind === 'pods' ? this.podTabs : this.resourceTabs,
            (tab, index) => (index === 0 ? basePath : `${basePath}/${tab}`)
          )
          const breadcrumb = [
            {
              label: msgs.get('search.label', this.context.locale),
              url: `/multicloud/search${_.get(location, 'state.search', '')}`
            }
          ]
        ...
        getResourceData() {
          ...
          const selfLink =
            '/' +
            baseSegments.slice(baseSegments.indexOf(cluster) + 1).join('/')
          const linkSegments = selfLink.split('/')
          const name = baseSegments[baseSegments.length - 1]
          const kind = baseSegments[baseSegments.length - 2]
          const isNamespaced =
            baseSegments[baseSegments.length - 4] === 'namespaces'
          const namespace = isNamespaced
            ? baseSegments[baseSegments.length - 3]
            : undefined
          const api = isNamespaced
            ? linkSegments
              .slice(0, linkSegments.indexOf('namespaces'))
              .join('/')
            : linkSegments.slice(0, linkSegments.indexOf(kind)).join('/')
        ...
        createBasePath() {
          ...
          const segments = location.pathname.split('/')
          this.tabs.find(tab => tab === segments[segments.length - 1]) &&
            segments.pop() // remove the tab path if present
        ...
        render() {
          ...
          return (
            <Query
              query={GET_RESOURCE}
              variables={{ selfLink, namespace, kind, name, cluster }}
            >
              {({ data, loading }) => (
                <Component
                  {...this.props}
                  {...resourceData}
                  resourceJson={data.getResource || {}}
