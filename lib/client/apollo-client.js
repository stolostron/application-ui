...
const typeDefs = gql`
  ...
  type Mutation {
    ...
    updateQueryTabs(__typename: String, unsavedCount: Int, openedTabName: String, data: JSON, tabs: [JSON]): JSON
    updateSingleQueryTab(openedTabName: String, searchText: String): JSON
    removeSingleQueryTab(queryName: String): JSON
  }
`
  ...
  resolvers: {
    Mutation: {
      ...
      updateQueryTabs: (_, variables, { cache }) => {
        ...
        const data = client.readQuery({query: GET_SEARCH_TABS})
      ...
      updateSingleQueryTab: (_, variables, { cache }) => {
        const data = client.readQuery({query: GET_SEARCH_TABS})
        const tabs = lodash.get(data, 'searchQueryTabs.tabs', [])
        const index = tabs.findIndex(tab => tab.queryName === variables.openedTabName)
        if (index !== -1) {
      ...
      removeSingleQueryTab: (_, variables, { cache }) => {
        const data = client.readQuery({query: GET_SEARCH_TABS})
        const tabs = lodash.get(data, 'searchQueryTabs.tabs', [])
        const index = tabs.findIndex(tab => tab.queryName === variables.queryName)
        let newSearchText = ''
        if (index !== -1 && tabs.length > 1) {
          tabs.splice(index, 1)
          if (index === 0) {
            data.openedTabName = tabs[0].queryName
            newSearchText = tabs[0].searchText
          } else {
            data.openedTabName = tabs[index -1].queryName
            newSearchText = tabs[index -1].searchText
          }
        }
        cache.writeData({ data })
        cache.writeData({ data: {
