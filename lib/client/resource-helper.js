...
export const getYamlEdit = ({ name, namespace, __typename, apiVersion }) => {
  const resourceKind = _.find(RESOURCE_TYPES, { kind: __typename })
  const resourceApi = resourceKind ? resourceKind.apiVersion : 'v1'
  const apiversion = apiVersion || resourceApi
