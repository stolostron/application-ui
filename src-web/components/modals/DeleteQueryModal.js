...
class SaveAndEditQueryModal extends React.PureComponent {
  ...
  makeApolloQuery = (name, client) => {
    apolloClient.deleteUserQueries( { name } )
      .then(result => {
        if (result.data.deleteQuery.errors && result.data.deleteQuery.errors.length > 0){
          this.setState({ errorMessage: result.data.deleteQuery.errors[0], loading: false })
        } else {
      ...
      {client => (
        <div className={'save-and-edit-query-modal'}>
          <Modal
            ...
            modalHeading={ msgs.get('modal.query.delete.heading', this.context.locale) }
            primaryButtonText={ msgs.get('actions.delete.search', this.context.locale) }
